...
for cars_area_elem in cars_area_elems:
    ...
    for model_name_elem, brand_name_elem, price_elem in zip(cars_info_elems, cars_info_elems, cars_price_elems):
        ...
        brand_name = a_tag_model.text.strip()
        splitname = brand_name.split(' ', 1)
        title = splitname[1]
        ...
        price_text = a_tag_price.text.strip()
...
# 6. JSON으로 저장
with open("electric_cars_price.json", "w", encoding="utf-8") as json_file:
